--This script is primarily (solely?) used to load scenegraph data into EMs
--required to make EMs
--[[
    Now loaded by the ScriptingEngine
include_lua('../Encounter_Manager.lua')
include_lua('../Global_Persistents.lua')
include_lua('../Global_NPC_Extras.lua')
]]
...
function spinSpawners()
    ...
    for i = 1, totalCount do
        ...
        MapEncPool[i].EncounterName = MapInstance.GetSpawnerName(i-1)
...
function VectorDistanceFloor(v1, v2)
...
function VectorDistanceCeil(v1, v2)
    local calcX = v1.x - v2.x
    local calcY = v1.y - v2.y
    local calcZ = v1.z - v2.z
