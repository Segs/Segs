 ...
 * Character Methods
 */
uint32_t            getLevel(Character &src) { return src.m_char_data.m_level; }
...
void                setEnd(Character &src, uint32_t val) { src.m_char_data.m_current_attribs.m_Endurance = val; }
uint64_t            getLastCostumeId(Character &src) { return src.m_char_data.m_last_costume_id; }
void                setLastCostumeId(Character &src, uint64_t val) { src.m_char_data.m_last_costume_id = val; }
const QString &     getOrigin(Character &src) { return src.m_char_data.m_origin_name; }
const QString &     getClass(Character &src) { return src.m_char_data.m_class_name; }
const QString &     getMapName(Character &src) { return src.m_char_data.m_mapName; }
void                setMapName(Character &src, const QString &val) { src.m_char_data.m_mapName = val; }
uint32_t            getXP(Character &src) { return src.m_char_data.m_experience_points; }
uint32_t            getDebt(Character &src) { return src.m_char_data.m_experience_debt; }
void                setDebt(Character &src, uint32_t val) { src.m_char_data.m_experience_debt = val; }
const QString &     getTitles(Character &src) { return src.m_char_data.m_titles[3]; }
void                toggleLFG(Character &src) { src.m_char_data.m_lfg = !src.m_char_data.m_lfg; }
uint32_t            getInf(Character &src) { return src.m_char_data.m_influence; }
void                setInf(Character &src, uint32_t val) { src.m_char_data.m_influence = val; }
void                setPowerLevel(Character &src, uint32_t val) { src.m_char_data.m_power_level = val; }

void toggleAFK(Character &src, const QString &msg)
{
    src.m_char_data.m_afk = !src.m_char_data.m_afk;
    if(src.m_char_data.m_afk)
        src.m_char_data.m_afk_msg = msg;
}

void setXP(Character &src, uint32_t val)
{
    src.m_char_data.m_experience_points = val;
...
{
    src.m_char_data.m_has_titles = prefix || !generic.isEmpty() || !origin.isEmpty() || special.isEmpty();
    if(!src.m_char_data.m_has_titles)
      return;

    src.m_char_data.m_has_the_prefix = prefix;
    src.m_char_data.m_titles[0] = generic;
    src.m_char_data.m_titles[1] = origin;
    src.m_char_data.m_titles[2] = special;
}
