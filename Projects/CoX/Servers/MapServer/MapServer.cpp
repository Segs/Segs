...
void MapServer::on_client_map_xfer(ClientMapXferMessage *ev)
{
    ...
    {
        m_current_map_xfers.insert(std::pair<uint64_t, uint32_t>(ev->m_data.m_session, ev->m_data.m_map_idx));
    }
    else
    {
...
uint32_t MapServer::session_map_xfer_idx(uint64_t session_token)
{
