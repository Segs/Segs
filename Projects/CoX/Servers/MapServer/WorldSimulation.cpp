...
struct CritterSpawnLocator
{
    ...
    CritterSpawnLocator(QHash<QString, CritterSpawnLocations> *spawn_def_hash) : m_spawn_def(spawn_def_hash) {}
...
struct SpawnPointLocator
{
    ...
    SpawnPointLocator(QMultiHash<QString, glm::mat4> *targets) : m_targets(targets) {}
    bool operator()(SceneNode *n, const glm::mat4 &v)
    {
        if (!n->m_properties)
            return true;
...
struct NavPointLocator
{
    ...
    NavPointLocator(std::vector<NavNode> &targets, const char *type) : m_targets(targets), m_beacon_type(type) {}
    bool operator()(SceneNode *n, const glm::mat4 &v)
    {
        if (!n->m_properties)
            return true;
...
struct MapXferLocator
{
    ...
    MapXferLocator(QHash<QString, MapXferData> *targets) : m_targets(targets) {}
    bool operator()(SceneNode *n, const glm::mat4 &v)
    {
        if (n->m_properties)
            return true;
...
void build_static_collision_meshes(World *world)
{
    ...
    //      apply physics material to collider (ice/water/mud/etc. )
}
void spawn_critters(MapSceneGraph *map_scene_graph, World *instance)
{
    ...
    for (auto r : res)
    {
        ...
        instance->critterGenerators().m_generators.insert(cg.m_generator_name, cg);
        ++count;
    }
}
...
{
    NpcCreator creator;
    creator.generators   = &instance->npcGenerators();
    creator.map_instance = instance;
    map_scene_graph->visitNodesFromRoots(creator);
}
QMultiHash<QString, glm::mat4> getSpawnPoints(MapSceneGraph *map_scene_graph)
{
...
bool World::start(const QString &scenegraph_path)
{
    ...
    TIMED_LOG(
        ...
        "Spawning npcs");
    TIMED_LOG({ build_static_collision_meshes(this); }, "Preparing static collsion meshes");

    TIMED_LOG({ generate_navigation_graphs(this); }, "Preparing navigation graph");
...
void World::update(const ACE_Time_Value &tick_timer)
{
    ...
    m_time_of_day +=
        4.8f * ((float(delta.msec()) / 1000.0f) / (60.0f * 60)); // 1 sec of real time is 48s of ingame time
...
glm::vec3 World::closest_safe_location(glm::vec3 v) const
{
    ...
    if (m_all_spawners.contains("NewPlayer"))
        ...
        loc = m_all_spawners.values(
            "LinkFrom_Monorail_Red")[rand() % m_all_spawners.values("LinkFrom_Monorail_Red").size()][3];
    else if (m_all_spawners.contains("LinkFrom_Monorail_Blue"))
        loc = m_all_spawners.values(
            "LinkFrom_Monorail_Blue")[rand() % m_all_spawners.values("LinkFrom_Monorail_Blue").size()][3];
...
void World::checkPowerTimers(Entity *e, uint32_t msec)
{
    ...
    for (auto rpow_idx = e->m_recharging_powers.begin(); rpow_idx != e->m_recharging_powers.end();
         /*rpow_idx updated inside loop*/)
    {
...
{
    if (e->m_type == EntType::PLAYER && getHP(*e->m_char) == 0.0f)
    {
...
void World::collisionStep(Entity *e, uint32_t /*msec*/)
{
    if (e->m_player != nullptr && !e->m_map_swap_collided)
    {
        ...
        {
            // TODO: This needs to check against the trigger plane for transfers. This should be part of the wall
            // objects geobin. Also need to make sure that this doesn't cause players to immediately zone after being
            // spawned in a spawnLocation near a zoneline.
            if ((e->m_entity_data.m_pos.x >= i.value().m_position.x - 20 &&
                 e->m_entity_data.m_pos.x <= i.value().m_position.x + 20) &&
                (e->m_entity_data.m_pos.y >= i.value().m_position.y - 20 &&
                 e->m_entity_data.m_pos.y <= i.value().m_position.y + 20) &&
                (e->m_entity_data.m_pos.z >= i.value().m_position.z - 20 &&
                 e->m_entity_data.m_pos.z <= i.value().m_position.z + 20))
            {
...
void WorldPhysics::update(float dt)
{
    ...
    auto first_inactive =
        std::stable_partition(m_per_entity_data.begin(), m_per_entity_data.end(),
                              [](const PhysicsState &a) -> bool { return glm::length2(a.m_velocity) != 0.0f; });
...
{
    auto iter = std::find_if(m_per_entity_data.begin(), m_per_entity_data.end(),
                             [e](const PhysicsState &s) -> bool { return s.m_ent == e; });
