...
template <class T> static void destruction_is_an_error(T & /*v*/)
{
    assert(false);
}
...
void ScriptingEngine::registerTypes()
{
    ...
    m_private->m_lua.new_usertype<Clue>("Clue", sol::constructors<Clue()>(), "name",
                                        sol::property(&Clue::getName, &Clue::setName), "displayName",
                                        sol::property(&Clue::getDisplayName, &Clue::setDisplayName), "detail",
                                        sol::property(&Clue::getDetailText, &Clue::setDetailText), "iconFile",
                                        sol::property(&Clue::getIconFile, &Clue::setIconFile));

    m_private->m_lua.new_usertype<Contact>(
        "Contact",
        // 3 constructors
        sol::constructors<Contact()>(),
        // gets or set the value using member variable syntax
        "name", sol::property(&Contact::getName, &Contact::setName), "locationDescription",
        sol::property(&Contact::getLocationDescription, &Contact::setLocationDescription), "npcId", &Contact::m_npc_id,
        "currentStanding", &Contact::m_current_standing, "contactIdx", &Contact::m_contact_idx, "notifyPlayer",
        &Contact::m_notify_player, "taskIndex", &Contact::m_task_index, "hasLocation", &Contact::m_has_location,
        "location", &Contact::m_location, "confidantThreshold", &Contact::m_confidant_threshold, "friendThreshold",
        &Contact::m_friend_threshold, "completeThreshold", &Contact::m_complete_threshold, "canUseCell",
        &Contact::m_can_use_cell, "contactId", &Contact::m_contact_idx, "dialogScreenIdx", &Contact::m_dlg_screen,
        "settingTitle", &Contact::m_setting_title);

    m_private->m_lua.new_usertype<Destination>(
        "Destination", sol::constructors<Destination()>(), "pointIdx", &Destination::point_idx, "location",
        &Destination::location, "name", sol::property(&Destination::getLocationName, &Destination::setLocationName),
        "mapName", sol::property(&Destination::getLocationMapName, &Destination::setLocationMapName));

    m_private->m_lua.new_usertype<HideAndSeek>("HideAndSeek", sol::constructors<HideAndSeek()>(), "foundCount",
                                               &HideAndSeek::m_found_count);

    m_private->m_lua.new_usertype<Hunt>("Hunt", sol::constructors<Hunt()>(), "type",
                                        sol::property(&Hunt::getTypeString, &Hunt::setTypeString), "count",
                                        &Hunt::m_count);

    m_private->m_lua.new_usertype<QString>("QString", sol::constructors<QString(), QString(const char *)>());

    m_private->m_lua.new_usertype<RelayRaceResult>(
        "RelayRaceResult", sol::constructors<RelayRaceResult()>(), "segment", &RelayRaceResult::m_segment, "lastTime",
        &RelayRaceResult::m_last_time, "bestTime", &RelayRaceResult::m_best_time);

    m_private->m_lua.new_usertype<Souvenir>("Souvenir", sol::constructors<Souvenir()>(), "name",
                                            sol::property(&Souvenir::getName, &Souvenir::setName), "description",
                                            sol::property(&Souvenir::getDescription, &Souvenir::setDescription), "icon",
                                            sol::property(&Souvenir::getIcon, &Souvenir::setIcon)
                                            // Server will set the index for this
                                            //"souvenirIdx", &Souvenir::m_idx
    );

    m_private->m_lua.new_usertype<TaskEntry>("TaskEntry", sol::constructors<TaskEntry()>(), "dbId", &TaskEntry::m_db_id,
                                             "taskList", &TaskEntry::m_task_list, "resetSelectedTask",
                                             &TaskEntry::m_task_list);

    m_private->m_lua.new_usertype<Task>(
        "Task", sol::constructors<Task()>(), "dbId", &Task::m_db_id, "description",
        sol::property(&Task::getDescription, &Task::setDescription), "owner",
        sol::property(&Task::getOwner, &Task::setOwner), "detail", sol::property(&Task::getDetail, &Task::setDetail),
        "state", sol::property(&Task::getState, &Task::setState), "isComplete", &Task::m_is_complete, "inProgressMaybe",
        &Task::m_in_progress_maybe, "hasLocation", &Task::m_has_location, "isDetailInvalid", &Task::m_detail_invalid,
        "location", &Task::m_location, "finishTime", &Task::m_finish_time, "taskIdx", &Task::m_task_idx, "isAbandoned",
        &Task::m_is_abandoned, "unknownInt1", &Task::m_unknown_1, "unknownInt2", &Task::m_unknown_2, "boardTrain",
        &Task::m_board_train);

    m_private->m_lua.new_usertype<glm::vec3>("vec3", sol::constructors<glm::vec3(), glm::vec3(float, float, float)>(),
    ...
    m_private->m_lua["DateTime"]["SecsSince2000Epoch"] = [this]() { return getSecsSince2000Epoch(); };
    // MapInstance
    m_private->m_lua.new_usertype<MapInstance>("World", "new",
                                               sol::no_constructor // Not constructible from the script side.
    );
    // MapInstance
    m_private->m_lua.new_usertype<MapInstance>("MapInstance", "new",
                                               sol::no_constructor // Not constructible from the script side.
    );

    m_private->m_lua["MapInstance"]["AddNpc"] = [this](const char *npc_def, glm::vec3 &loc, glm::vec3 &ori,
                                                       int variation, const char *npc_name) {
    ...
    m_private->m_lua["MapInstance"]["AddEnemy"] = [this](const char *npc_def, glm::vec3 &loc, glm::vec3 &ori,
                                                         int variation, const char *npc_name, int level,
                                                         const char *faction_name, int rank) {
        MapInstance *mi           = m_private->m_lua["map"];
        QString      npc_def_name = QString::fromUtf8(npc_def);
        QString      name         = QString::fromUtf8(npc_name);
    ...
    m_private->m_lua["MapInstance"]["RemoveNpc"] = [this](int entityIdx) {
        World *      world = m_private->m_lua["world"];
        Entity *     e     = world->getEntity(entityIdx);
        if (e != nullptr)
        {
    ...
    m_private->m_lua["MapInstance"]["SetOnTickCallback"] =
        [this](int entityIdx, std::function<void(int64_t, int64_t, int64_t)> callback) {
            MapInstance *mi    = m_private->m_lua["map"];
            World *      world = mi->world();
            Entity *     e     = world->getEntity(entityIdx);
            if (e != nullptr)
            {
                LuaTimer timer;
    ...
    // MapClientSession
    m_private->m_lua.new_usertype<MapClientSession>(
        "MapClientSession",

        "new", sol::no_constructor, // The client links are not constructible from the script side.
        "m_ent", sol::readonly(&MapClientSession::m_ent), "admin_chat_message", sendChatMessage
    ...
    m_private->m_lua["MapClientSession"]["Contact_dialog"] =
        [this](const char *                                                                      message,
               sol::as_table_t<std::map<std::string, sol::as_table_t<std::vector<std::string>>>> buttons) {
            MapClientSession *        cl = m_private->m_lua["client"];
    ...
    m_private->m_lua["MapClientSession"]["ForceEntityLocation"] = [this](int entity_idx, glm::vec3 loc, glm::vec3 ori) {
        MapClientSession *cl    = m_private->m_lua["client"];
        World *           world = cl->m_current_map->world();
        Entity *          e     = world->getEntity(entity_idx);
        if (e != nullptr)
        {
            e->m_world->m_physics.forcePositionAndOrientation(*e, loc, ori);
            QString msg = QString("Setting entiry %1 orientation to x: %2 y: %3 z: %4")
                              .arg(entity_idx)
                              .arg(ori.x)
                              .arg(ori.y)
                              .arg(ori.z);
    ...
    m_private->m_lua["MapClientSession"]["SetNpcStore"] = [this](int entity_idx, const char *store_name,
                                                                 int item_count) {
        MapClientSession *cl    = m_private->m_lua["client"];
        World *           world = cl->m_current_map->world();
        Entity *          e     = world->getEntity(entity_idx);
    ...
    m_private->m_lua["MapClientSession"]["NpcMessage"] = [this](const char *channel, int entityIdx,
                                                                const char *message) {
        MapClientSession *cl  = m_private->m_lua["client"];
        QString           msg = QString::fromUtf8(message);
        QString           ch  = QString::fromUtf8(channel);
    ...
    m_private->m_lua["MapClientSession"]["SetOnTickCallback"] =
        [this](int entityIdx, std::function<void(int64_t, int64_t, int64_t)> callback) {
            MapClientSession *cl    = m_private->m_lua["client"];
    ...
    m_private->m_lua.new_usertype<Entity>(
        "Entity", "new", sol::no_constructor, // not constructible from the script side.
        sol::meta_function::garbage_collect, sol::destructor(destruction_is_an_error<Entity>), "abort_logout",
        abortLogout, "begin_logout", &Entity::beginLogout);
    m_private->m_lua.new_usertype<MapSceneGraph>(
        "MapSceneGraph", "new", sol::no_constructor // The client links are not constructible from the script side.
    );
    m_private->m_lua.script("function ErrorHandler(msg) return \"Lua call error:\"..msg end");
    m_private->m_lua["printDebug"] = [](const char *msg) { qCDebug(logScripts) << msg; };
...
void ScriptingEngine::callFuncWithMapInstance(MapInstance *mi, const char *name, int arg1)
{
    ...
    m_private->m_lua["world"]                = mi->world();
...
std::string ScriptingEngine::callFuncWithClientContext(MapClientSession *client, const char *name, int arg1)
{
    ...
    m_private->m_lua["world"]                = client->m_current_map->world();
    m_private->m_lua["contactDialogButtons"] = contactLinkHash;
...
std::string ScriptingEngine::callFuncWithClientContext(MapClientSession *client, const char *name, const char *arg1,
                                                       glm::vec3 loc)
{
    m_private->m_lua["client"]             = client;
    m_private->m_lua["map"]                = client->m_current_map;
...
std::string ScriptingEngine::callFunc(const char *name, Args&&... args)
{
    ...
    sol::protected_function_result result = funcwrap.call(std::forward<Args>(args)...);
    if (!result.valid())
    {
