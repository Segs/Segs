...
void GameHandler::dispatch( SEGSEvent *ev )
{
    ...
    case GameEventTypes::evIdle:
        on_idle((IdleEvent<GameLinkEvent> *)ev);
        break;
    case GameEventTypes::evDisconnectRequest:
        on_disconnect((DisconnectRequest<GameLinkEvent> *)ev);
        break;

    case GameEventTypes::evConnectRequest:
        on_connection_request((ConnectRequest<GameLinkEvent> *)ev);
        break;
...
void GameHandler::on_connection_request(ConnectRequest<GameLinkEvent> *ev)
{
    // TODO: disallow connects if server is overloaded
    ev->src()->putq(new ConnectResponse<GameLinkEvent>);
}
...
void GameHandler::on_idle(IdleEvent<GameLinkEvent> *ev)
{
    GameLink * lnk = (GameLink *)ev->src();
    lnk->putq(new IdleEvent<GameLinkEvent>);
}
void GameHandler::on_disconnect(DisconnectRequest<GameLinkEvent> *ev)
{
    GameLink * lnk = (GameLink *)ev->src();
    CharacterClient *client = (CharacterClient *)lnk->client_data();
    if(client)
